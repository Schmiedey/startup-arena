  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, w: number, h: number, t: number) => {
      ...
      if (isDark) {
        ...
        bgGrad.addColorStop(0.45, "rgba(10,8,20,1)");
      ...
      if (isDark && !mobile) {
        ...
        for (let i = 0; i < starCount; i++) {
          ...
          ctx.fillStyle = `rgba(200,180,220,${twinkle * 0.35})`;
          ctx.fill();
        }
      }
      ...
      if (isDark) {
        glowGrad.addColorStop(0, "rgba(220,60,30,0.2)");
        glowGrad.addColorStop(0.35, "rgba(180,40,20,0.08)");
        glowGrad.addColorStop(1, "rgba(0,0,0,0)");
      } else {
        glowGrad.addColorStop(0, "rgba(220,80,40,0.1)");
        glowGrad.addColorStop(0.35, "rgba(220,80,40,0.03)");
        glowGrad.addColorStop(1, "rgba(0,0,0,0)");
      }
      ctx.fillStyle = glowGrad;
      ctx.fillRect(0, horizonY - h * 0.3, w, h * 0.6);

      // 3D perspective grid floor
      const gridColor = isDark ? [220, 60, 30] : [220, 80, 40];
      const gridLines = mobile ? 14 : 28;
      ...
      for (let i = -vLines; i <= vLines; i++) {
        ...
        const edgeFade = 1 - Math.abs(normalX) * 0.5;
      ...
      // Floating geometric shapes (desktop, no reduced motion)
      if (!mobile && !prefersReducedMotion) {
        ...
        for (const shape of shapes) {
          ...
          const shapeAlpha = isDark ? 0.25 : 0.1;

          ctx.save();
          ctx.translate(sx, sy);
          ctx.rotate(rot * 0.3);
          ctx.strokeStyle = `rgba(${gridColor[0]},${gridColor[1]},${gridColor[2]},${shapeAlpha})`;
      ...
      // Ember particles
      const particleCount = mobile ? 15 : 45;
      for (let i = 0; i < particleCount; i++) {
        ...
        const pxFinal = px * w + (mobile ? 0 : Math.sin(t * 0.0008 + i) * 30 * drift) + (mobile ? 0 : parallaxX * 0.3);
        const pyFinal = py * h + (mobile ? 0 : parallaxY * 0.15);

        if (pyFinal < horizonY + 20) continue;

        const fadeIn = Math.min(1, (pyFinal - horizonY) / 80);
        const fadeOut = pyFinal > h * 0.9 ? (1 - (pyFinal - h * 0.9) / (h * 0.1)) : 1;
        const alpha = fadeIn * fadeOut * (isDark ? 0.65 : 0.3);
        const hue = 15 + (i % 12);
      ...
      // Subtle glow on select particles (no shadowBlur)
      if (isDark && !mobile) {
        for (let i = 0; i < 8; i++) {
          const seed = i * 4973 + 7;
          const px = ((seed * 13) % 1000) / 1000;
          ...
          const py = ((baseY - t * speed) % 1.2 + 1.2) % 1.2;
          const pxFinal = px * w + Math.sin(t * 0.0008 + i) * 30 * ((seed * 17) % 100 / 100 - 0.5) + parallaxX * 0.3;
          const pyFinal = py * h + parallaxY * 0.15;

          if (pyFinal < horizonY + 20 || pyFinal > h * 0.9) continue;

          const glowGrad = ctx.createRadialGradient(pxFinal, pyFinal, 0, pxFinal, pyFinal, 8);
          glowGrad.addColorStop(0, `hsla(${15 + (i % 12)},90%,60%,0.25)`);
          glowGrad.addColorStop(1, "rgba(0,0,0,0)");
          ctx.fillStyle = glowGrad;
          ctx.fillRect(pxFinal - 8, pyFinal - 8, 16, 16);
        }
      }
      ...
      // Atmospheric fog at bottom — subtle, lets grid show through
      const fogGrad = ctx.createLinearGradient(0, h * 0.82, 0, h);
      if (isDark) {
        fogGrad.addColorStop(0, "rgba(8,5,12,0)");
        fogGrad.addColorStop(1, "rgba(8,5,12,0.7)");
      } else {
        fogGrad.addColorStop(0, "rgba(245,240,235,0)");
        fogGrad.addColorStop(1, "rgba(245,240,235,0.8)");
      }
